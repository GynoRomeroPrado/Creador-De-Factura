#!/usr/bin/env python3
"""
Test simplificado para validar generación de facturas realistas (sin PDFs).

Valida que el generador funcione correctamente con los nuevos módulos sin
necesidad de generar PDFs (no requiere reportlab).
"""
...
def test_generacion_realista_json():
    """
    Test: Genera datos JSON de facturas realistas y valida características.
    """
    print("\n" + "="*80)
    print("TEST: Generación de Datos JSON Realistas (sin PDF)")
    print("="*80 + "\n")
    ...
    print("2. Generando datos JSON de facturas (10 facturas)...")
    print("-" * 80)
    ...
    for i in range(10):
        try:
            ...
            print(f"    • Dir emisor ({len(dir_emisor)} chars): {dir_emisor[:65]}...")
            print(f"    • Item ejemplo ({len(item_ejemplo)} chars): {item_ejemplo[:65]}...")
            ...
            if tiene_modo_realista:
                validaciones.append("✓ Modo realista")
            if len(dir_emisor) > 40:  # Direcciones realistas son más largas
                validaciones.append("✓ Dir detallada")
            if len(item_ejemplo) > 30:  # Descripciones realistas son más largas
                validaciones.append("✓ Desc detallada")
            if 'Lima' in dir_emisor or 'Lima' in dir_receptor:  # Direcciones tienen Lima
                validaciones.append("✓ Dir de Lima")

            # Verificar que la dirección incluye distrito y código postal
            tiene_codigo_postal = any(char.isdigit() for char in dir_emisor) and 'Lima' in dir_emisor
            if tiene_codigo_postal:
                validaciones.append("✓ CP incluido")
    ...
    if facturas_generadas:
        ...
        con_modo = sum(1 for fg in facturas_generadas if fg['factura'].get('modo_realista', False))
        ...
        con_lima = sum(1 for fg in facturas_generadas if 'Lima' in fg['factura']['emisor']['direccion'])
        ...
        print(f"  • Modo realista activo: {con_modo}/{total_val} ({con_modo/total_val*100:.0f}%)")
        ...
        print(f"  • Direcciones de Lima: {con_lima}/{total_val} ({con_lima/total_val*100:.0f}%)")

        # Estadísticas de longitud
        print("\nEstadísticas de longitud:")
        len_dirs = [len(fg['factura']['emisor']['direccion']) for fg in facturas_generadas]
        len_descs = [len(fg['factura']['items'][0]['descripcion']) for fg in facturas_generadas if fg['factura']['items']]

        print(f"  • Direcciones: min={min(len_dirs)}, max={max(len_dirs)}, promedio={sum(len_dirs)/len(len_dirs):.1f}")
        print(f"  • Descripciones: min={min(len_descs)}, max={max(len_descs)}, promedio={sum(len_descs)/len(len_descs):.1f}")
    ...
    if len(facturas_generadas) == 10 and len(errores) == 0:
        ...
        print("   • Múltiples tipografías asignadas")
        print("\n📊 Impacto:")
        print("   • Dataset más diverso y realista")
        print("   • Mejor entrenamiento para LayoutLMv3")
        print("   • PDFs más profesionales y variados")
        print("\n💡 Próximo paso:")
        print("   • Instalar reportlab para generar PDFs: pip install reportlab")
        print("   • Ejecutar test_facturas_realistas.py para validar PDFs completos")
        return 0
    elif len(facturas_generadas) >= 7:
        print("✅ La mayoría de las pruebas pasaron")
        print(f"   {len(facturas_generadas)}/10 facturas generadas correctamente")
        return 0
    else:
...
def test_comparacion_modo_basico_vs_realista():
    ...
    print(f"   • Dirección ({len(factura_basica['emisor']['direccion'])} chars): {factura_basica['emisor']['direccion'][:60]}...")
    print(f"   • Item 1 ({len(factura_basica['items'][0]['descripcion'])} chars): {factura_basica['items'][0]['descripcion'][:60]}...")
    ...
    print(f"   • Dirección ({len(factura_realista['emisor']['direccion'])} chars): {factura_realista['emisor']['direccion'][:60]}...")
    print(f"   • Item 1 ({len(factura_realista['items'][0]['descripcion'])} chars): {factura_realista['items'][0]['descripcion'][:60]}...")
    ...
    mejora = "SI" if (len_dir_realista > len_dir_basica and len_desc_realista > len_desc_basica) else "NO"
    print(f"\n✅ Mejora de realismo detectada: {mejora}")
...
if __name__ == '__main__':
    # Test principal
    result = test_generacion_realista_json()
